...
namespace WebAppCA.Controllers
{
    public class DoorController : Controller
    {
        ...
        public IActionResult AddDoor(string doorName, uint deviceID, int portNumber)
        {
            ...
            {
                // Initialiser le SDK
                int result = DeviceController.BS2_Initialize();
                if (result != 0) throw new Exception($"Erreur SDK: {result}");

                // Connecter à l'appareil
                IntPtr deviceContext;
                result = DeviceController.BS2_ConnectDevice(IntPtr.Zero, deviceID, out deviceContext);
